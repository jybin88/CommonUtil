...
public class PermissionUtil {
     ...
     * @param permissions
     */
    public static void request(final Activity pActivity, final IPermissionClient client, final String... permissions) {
        if (client == null) {
            throw new IllegalArgumentException("client is null");
        }

        getPermissionObservable(pActivity, permissions).subscribe(new Action1<Boolean>() {
            @Override
            public void call(Boolean granted) {
                if (granted) {
                    client.onSuccess(pActivity);
                } else {
                    client.onFailure(pActivity);
                }
            }
        });
    }
